...
const PlacesScreen = () => {
  ...
  const applyFilter = (filter) => {
    ...
    switch (filter) {
      case "cost:High to Low":
        searchPlaces.forEach((val) => val.properties.sort(compare));
        setSortedData(searchPlaces);
        break;
      case "cost:Low to High":
        searchPlaces.forEach((val) => val.properties.sort(comparison));
        setSortedData(searchPlaces);
        break;
    }
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setLoading(false);
    }, 1000); // Set a delay for loading state, can be adjusted
  }, []);
      ...
      >
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Sort and Filter</Text>
